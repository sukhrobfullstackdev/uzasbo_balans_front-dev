...
const GetListTable = () => {
  ...
  const tableList = useSelector((state) => state.PermanentAssetRetirementList);
  ...
  const deleteRowHandler = id => {
    setLoading(true);
    PermanentAssetRetirementServices.delete(id)
      .then(res => {
        if (res.status === 200) {
  ...
  const acceptHandler = (id) => {
    setLoading(true);
    PermanentAssetRetirementServices.Accept(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  const declineHandler = (id) => {
    setLoading(true);
    PermanentAssetRetirementServices.cancel(id)
      .then((res) => {
        if (res.status === 200) {
  ...
  const printHandler = (id) => {
    setLoading(true);
    PermanentAssetRetirementServices.printById(id)
      .then(res => {
        if (res.status === 200) {
  ...
  const columns = [
    ...
    {
      title: t("RetirementDetails"),
      dataIndex: "RetirementDetails",
      sorter: true,
      width: 150,
    },
    {
      title: t("PermanentAssetQuitType"),
      dataIndex: "PermanentAssetQuitType",
      sorter: true,
      width: 250,
    },
   
    {
      ...
      width: 150,
      render: (_, record) => {
        if (record.Status === 2 || record.StatusID === 9) {
          return (
            <Tag color='#87d068'>
              {record.Status}
            </Tag>
