...
const UpdatePermanentAssetAgeing = (props) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const [allowedTrans,] = await Promise.all([
                PermanentAssetAgeingServices.getById(docId ? docId : 0),             
            ]);
    ...
    const onMainFormFinish = (values) => {
        setLoader(true);
        PermanentAssetAgeingServices.update({
            ...allowTrans, ...values,
            ...
            .then((res) => {
                if (res.status === 200) {
                    setLoader(false);
                    history.push(`/PermanentAssetAgeing`);
    ...
    return (
        <MainCard title={t("PermanentAssetAgeing")}>
...
export default React.memo(UpdatePermanentAssetAgeing);