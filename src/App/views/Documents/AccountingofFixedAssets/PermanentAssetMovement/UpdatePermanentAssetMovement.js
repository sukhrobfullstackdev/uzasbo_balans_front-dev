...
const UpdatePermanentAssetMovement = (props) => {
   ...
   // const [CurrencySum, setCurrencySum] = useState();
    ...
    useEffect(() => {
        const fetchData = async () => {
            const [allowedTrans, documentList, accDbList] = await Promise.all([
                PermanentAssetMovementServices.getById(props.match.params.id ? props.match.params.id : 0),
    ...
    const onMainFormFinish = (values) => {
        setLoader(true);
        PermanentAssetMovementServices.update({
            ...allowTrans, ...values,
            ...
            .then((res) => {
                if (res.status === 200) {
                    setLoader(false);
                    history.push(`/PermanentAssetMovement`);
  ...
  ///ResponsiblePersonDbModal modal
    ...
    return (
        <MainCard title={t("PermanentAssetMovement")}>
...
export default React.memo(UpdatePermanentAssetMovement);