...
import { Table, Tag, Space, Modal, Dropdown, Menu, Tooltip, Input, Form,  } from 'antd';
...
import CheckDocModal from './CheckDocMadal';
...
const GetListTable = () => {
  ...
  const deleteRowHandler = id => {
    setLoading(true);
    IncomeCashOrderServices.delete(id)
      .then(res => {
        if (res.status === 200) {
  ...
  const openCheckDocModal = (id ) => {
    setRowId(id);
    setCheckDocModalVisible(true);
  }

  const acceptHandler = (record) => {
    if (record.AccID === 43) {
         setCheckDocModalVisible(true);
         setRowId(record.ID);
    } else if(record.AccID !== 43){
      IncomeCashOrderServices.Accept(record.ID)
        .then((res) => {
          if (res.status === 200) {
  ...
  const modalAcceptHandler = (rowId) => {
    console.log(rowId);
    setLoading(true);
    IncomeCashOrderServices.Accept(rowId)
      .then((res) => {
        if (res.status === 200) {
          Notification('success', t('accepted'));
          dispatch(getListStartAction({
  ...
  const printHandler = (id) => {
    setLoading(true);
    IncomeCashOrderServices.printById(id)
      .then(res => {
        if (res.status === 200) {
  ...
  const declineHandler = (id) => {
    setLoading(true);
    IncomeCashOrderServices.cancel(id)
      .then((res) => {
        if (res.status === 200) {
