...
const UpdateFoodstuffIntake = (props) => {
    ...
    const [foodStuffIntake, setInventHoldIntake] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            const [foodStuffIntake, allowTransInDoc, allowTransInDoc2] = await Promise.all([
                FoodstuffIntakeServices.getById(docId),
                HelperServices.getAllowedTransactionInDocument(211),
                HelperServices.getAllowedTransactionInDocument2(114),
            ]);

            setInventHoldIntake(foodStuffIntake.data);
            setAllowTransList(allowTransInDoc.data);
            setAllowTransList2(allowTransInDoc2.data)
            setTables(foodStuffIntake.data.ChargesTables);
            setTables2(foodStuffIntake.data.Tables);

            mainForm.setFieldsValue({
                ...foodStuffIntake.data,
                Date: moment(foodStuffIntake.data.Date, 'DD.MM.YYYY'),
            });
    ...
    const onMainFormFinish = (values) => {
        ...
        // values.Tables = settlementAddTableModalData;
        setLoader(true);
        InventoryHoldingIntakeServices.update({
            ...foodStuffIntake, ...values,
    ...
    const editTableData = (data) => {
        ...
        data.map(data => {
            if (data.Status !== 3) {
                totalsum += data.Sum
                totalcur += data.IntakeDetails
            }
            return null;
        })
        mainForm.setFieldsValue({
            [`Sum`]: totalsum,
            [`IntakeDetails`]: totalcur,
        });
        setTables(data);
    };
...
export default React.memo(UpdateFoodstuffIntake);