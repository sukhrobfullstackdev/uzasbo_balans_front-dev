...
const PaymentOrder = ({ match }) => {
    const { t } = useTranslation();
    const dispatch = useDispatch();
    const [filterForm] = Form.useForm();
    const location = useLocation();

    const paymentOrderList = useSelector((state) => state.paymentOrderList);

    let tableData = paymentOrderList.listSuccessData?.rows;
    let total = paymentOrderList.listSuccessData?.total;
    let pagination = paymentOrderList?.paginationData;
    let filter = paymentOrderList?.filterData;
    ...
    const [filterModal, setFilterModal] = useState(paymentOrderList.filterType);
