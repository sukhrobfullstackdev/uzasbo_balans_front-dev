...
const TableData = ({ tableData, total, match, reduxList, OrganizationAccountID }) => {
    ...
    const columns = [
        ...
        {
            title: t("PayeeSettlement"),
            dataIndex: "PayeeSettlement",
            key: "PayeeSettlement",
            sorter: true,
            width: 200
        },
                            ...
                            overlay={<Menu items={[
                                {
                                    key: 'edit',
                                    label: (
                                        <Link to={`${location.pathname}/edit/${record.ID}?OrganizationAccountID=${OrganizationAccountID}`}>
                                ...
                                },
                                {
                                    key: 'documentHistory',
                                    label: (
                                        <span onClick={() => documentHistoryHandler(record.ID, 129)}>
                                            <i className="feather icon-folder action-icon" aria-hidden="true" />&nbsp;
                                            {t("documentHistory")}
                                        </span>
                                    ),
                                },
    ...
    const printByType = (e, id) => {
        loading = true;
        PaymentRegisterServices.printType(id, e.key)
            .then((res) => {
                if (res.status === 200) {
                    loading = false;
    ...
    function postLoadKey(id, vo, reload) {
        ...
        window.CAPIWS.callFunction({
            ...
            if (data.success) {
                ...
                PaymentRegisterServices.postSignedData(signData)
                    .then(res => {
                        if (res.status === 200) {
    ...
    const showSendModal = (id) => {
        ...
        confirm({
            ...
            onOk: () => {
                ...
                PaymentRegisterServices.getHash(id)
                    .then(res => {
                        if (res.status === 200) {
    ...
    const acceptHandler = (id) => {
        confirm({
            ...
            onOk: () => {
                setTableLoading(true);
                PaymentRegisterServices.Accept(id, dateOfAcception)
                    .then((res) => {
                        if (res.status === 200) {
    ...
    const cancelHandler = (id) => {
        confirm({
            ...
            onOk: () => {
                setTableLoading(true);
                PaymentRegisterServices.cancel(id)
                    .then((res) => {
                        if (res.status === 200) {
                            Notification('success', t('success-msg'));
                            setTableLoading(false);
    ...
    const prepareToSendHandler = (id) => {
        confirm({
            ...
            onOk: () => {
                setTableLoading(true);
                PaymentRegisterServices.prepareForSend(id)
                    .then((res) => {
                        if (res.status === 200) {
                            Notification('success', t('success-msg'));
                            setTableLoading(false);
    ...
    const changeFinYearHandler = (id) => {
        confirm({
            ...
            onOk: () => {
                setTableLoading(true);
                PaymentRegisterServices.changeFinYear(id)
                    .then((res) => {
                        if (res.status === 200) {
                            Notification('success', t('success-msg'));
                            setTableLoading(false);
    ...
    const documentHistoryHandler = (id, tableID) => {
        setHistoryParams({ id, tableID });
    ...
    const deleteModalHandler = (id) => {
        confirm({
            ...
            onOk: () => {
                setTableLoading(true);
                PaymentRegisterServices.delete(id)
                    .then((res) => {
                        if (res.status === 200) {
                            Notification('success', t('success-msg'));
                            setTableLoading(false);
